                 ...
                 "housing",pMotherLogical,pMany,pCopyNo),fConstructor(c)
{
 ...
 G4double fScint_rad = sqrt(fScint_x*fScint_x+fScint_y*fScint_y);
 G4double housing_rad = sqrt(housing_x*housing_x+housing_y*housing_y);
 fScint_tubs = new G4Tubs("scint_tubs", 0.*cm, fScint_rad/2., fScint_z/2., 0.*deg,360.*deg);
 fHousing_tubs = new G4Tubs("housing_tubs", 0.*cm, housing_rad/2., housing_z/2.,  0.*deg,360.*deg);
 fScint_log = new G4LogicalVolume(fScint_tubs, G4Material::GetMaterial("LXe"), "scint_log", 0, 0, 0);
 fHousing_log = new G4LogicalVolume(fHousing_tubs, G4Material::GetMaterial("Al"), "housing_log", 0, 0, 0);
 new G4PVPlacement(0, G4ThreeVector(), fScint_log, "scintillator", fHousing_log, false, 0);
  ...
  G4double dx = sqrt(fScint_x*fScint_x+fScint_y*fScint_y)/fNx;//////////////////
  G4double dy = sqrt(fScint_x*fScint_x+fScint_y*fScint_y)/fNy;//////////////////
 
  G4double x,y,z; //////////////
  G4double xmin = -sqrt(fScint_x*fScint_x+fScint_y*fScint_y)/2.-dx/2;///////////
  G4double ymin = -sqrt(fScint_x*fScint_x+fScint_y*fScint_y)/2.-dy/2;///////////
  //////////
  G4int k=1;/////////
